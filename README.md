...
The `post_deploy_smoke` job **fails** (red check) when smoke does not pass. Because GitHub does not pass **job outputs** from a failed job to dependents, a small **`smoke_outcome`** job always runs afterward: it downloads a **`smoke-outcome`** artifact written by the smoke job and republishes `smoke_ok` so `rollback_pages` can run. A `smoke_gate` job then fails the whole workflow unless smoke passed **or** rollback deploy completed. When the gate passes, `finalize_deployment` prints an `OK` marker.
