...
#### Automatic rollback (always to a `vX.Y.Z` tag)

GitHub Pages does **not** provide a native “revert this deployment” API after a bad publish. Rollback runs when smoke fails **and** the deploy is rollback-eligible: **published Release**, ref **`vX.Y.Z`** / **`X.Y.Z`**, or **`push` / `workflow_dispatch`** on the **default branch** (for example `master`). Tags are ordered by **creation time** (`git tag -l 'v*.*.*' --sort=-creatordate`). If the failed deploy was **from a tag**, the workflow redeploys the **chronologically previous** tag in that list. If the failed deploy was **from a branch ref**, there is no “current tag” on the list, so it redeploys the **newest** matching tag (first entry — last tagged release). There is **no second Playwright run** after rollback.
